...
use crate::{
	controller::UiEvent,
	layout::{CELL_SIZE, GRID_ORIGIN_X, GRID_ORIGIN_Y, PROMOTION_LEFT_CELLS, PROMOTION_PIECES, PROMOTION_TOP_CELLS, RESET_BUTTON},
	state::{GameState, InputState, UiState},
};
...
pub fn mouse_over_rect(input: &InputState, rect: Rect) -> bool {
	input.mouse_x >= rect.x && input.mouse_x <= rect.x + rect.w && input.mouse_y >= rect.y && input.mouse_y <= rect.y + rect.h
}
