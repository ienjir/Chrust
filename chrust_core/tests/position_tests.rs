...
fn square_string_too_short_returns_error() {
    assert!(matches!(
        convert_square_string_to_square("a"),
        Err(FenError::SquareLenghtIsnt2Wide(1))
    ));
}
...
fn square_string_too_long_returns_error() {
    assert!(matches!(
        convert_square_string_to_square("a12"),
        Err(FenError::SquareLenghtIsnt2Wide(3))
    ));
}
...
fn square_string_empty_returns_error() {
    assert!(matches!(
        convert_square_string_to_square(""),
        Err(FenError::SquareLenghtIsnt2Wide(0))
    ));
}
...
fn square_string_invalid_file_returns_error() {
    assert!(matches!(
        convert_square_string_to_square("i4"),
        Err(FenError::InvalidFile('i'))
    ));
    assert!(matches!(
        convert_square_string_to_square("z1"),
        Err(FenError::InvalidFile('z'))
    ));
}
...
fn square_string_rank_zero_returns_error() {
    ...
    assert!(matches!(
        convert_square_string_to_square("a0"),
...
fn square_string_rank_nine_returns_error() {
    assert!(matches!(
        convert_square_string_to_square("a9"),
        Err(FenError::InvalidRank('9'))
    ));
}
...
fn square_string_non_digit_rank_returns_error() {
    assert!(matches!(
        convert_square_string_to_square("ax"),
        Err(FenError::InvalidRank('x'))
    ));
}
...
fn fen_with_too_few_parts_returns_error() {
    ...
    assert!(matches!(
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -"),
        Err(FenError::MissingFenParts)
    ));
}
...
fn fen_with_too_many_parts_returns_error() {
    assert!(matches!(
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1 extra"),
        Err(FenError::MissingFenParts)
    ));
}
...
fn fen_with_invalid_piece_char_returns_error() {
    ...
    assert!(matches!(
        load_position_from_fen("rnbqkXnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"),
...
fn fen_with_invalid_side_to_move_returns_error() {
    assert!(matches!(
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR x KQkq - 0 1"),
        Err(FenError::NotAValideSide)
    ));
}
...
fn fen_with_invalid_castling_char_returns_error() {
    assert!(matches!(
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w XQkq - 0 1"),
        Err(FenError::InvalidCastlingRights('X'))
    ));
}
...
fn fen_with_invalid_en_passant_square_returns_error() {
    ...
    assert!(matches!(
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq z9 0 1"),
        Err(_)
    ));
}
...
fn fen_side_to_move_white() {
    let pos =
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1").unwrap();
    assert_eq!(pos.side_to_move, Side::White);
}

#[test]
fn fen_side_to_move_black() {
    let pos = load_position_from_fen("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1")
        .unwrap();
...
fn fen_castling_no_rights() {
    let pos =
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w - - 0 1").unwrap();
...
fn fen_castling_white_only() {
    let pos =
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQ - 0 1").unwrap();
...
fn fen_castling_white_kingside_only() {
    let pos =
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w K - 0 1").unwrap();
    assert!(pos.castle[0]);
    assert!(!pos.castle[1]);
    assert!(!pos.castle[2]);
    assert!(!pos.castle[3]);
...
fn fen_castling_black_queenside_only() {
    let pos =
        load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w q - 0 1").unwrap();
    assert!(!pos.castle[0]);
    assert!(!pos.castle[1]);
    assert!(!pos.castle[2]);
    assert!(pos.castle[3]);
...
fn fen_en_passant_d6_is_set() {
    ...
    let pos =
        load_position_from_fen("rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 2")
            .unwrap();
...
fn fen_starting_position_white_pawns_on_rank_2() {
    ...
    for sq in 8..16u8 {
        assert_eq!(
            ...
            "expected white pawn on square {sq}"
        );
    }
}
...
fn fen_starting_position_black_pawns_on_rank_7() {
    ...
    for sq in 48..56u8 {
        assert_eq!(
            ...
            "expected black pawn on square {sq}"
        );
    }
}
...
fn fen_digit_skips_empty_squares_correctly() {
    ...
    assert!(
        pos.board.iter().all(|sq| sq.is_none()),
...
fn fen_halfmove_clock_is_not_parsed_stays_zero() {
    ...
    let pos = load_position_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 7 42")
        .unwrap();
    assert_eq!(
        pos.halfmove_clock, 0,
        "halfmove_clock is not parsed (known bug)"
    );
    assert_eq!(
        pos.fullmove_number, 0,
        "fullmove_number is not parsed (known bug)"
    );
}
