...
impl Position {
	...
	match mv.move_kind {
		...
		self.board[mv.to_square as usize] = Some(ColoredPiece {
		...
		self.board[rook_to as usize] = Some(ColoredPiece {
		    piece: Piece::Rook,
		    side: piece.side,
		});
	    }
	...
	if mv.colored_piece.piece == Piece::Pawn
	    || matches!(mv.move_kind, MoveKind::Capture | MoveKind::EnPassant { .. })
	{
